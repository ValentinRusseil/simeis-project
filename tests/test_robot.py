...
class Game:
    ...
    def init_game(self):
        ...
        if len(status["ships"]) == 0:
            ...
            player_money_after_buy = status["money"]
            
            #assert we have less money now
            assert player_money_after_buy< player_money
            print("base money : {} \n actual money: {}".format(player_money,player_money_after_buy))
            player_money = player_money_after_buy
    ...
    def go_mine(self):
        ...
        player_money = self.get(f"/player/{self.pid}")["money"]
        ...
        if not check_has(ship["modules"], "modtype", modtype):
            ...
            player_money_after_buy = self.get(f"/player/{self.pid}")["money"]
            #ensure the money as correclty been substracted from player money before transaction
            assert player_money>player_money_after_buy
            print("base money : {} \n actual money: {}".format(player_money,player_money_after_buy))
            player_money = player_money_after_buy
        ...
        if ship["position"] != nearest["position"]:
            ...
            # we ensure we have traveled right to the planet
            assert ship["position"],nearest["position"]
            print("voyage sur la planète ", nearest)
    ...
    def go_sell(self):
        ...
        if ship["position"] != station["position"]:
            ...
            assert ship["position"], station["position"]
            print("Retour à la station")
            
        #get cargo resources
        charge = ship["cargo"]["resources"]
        assert len(charge.items())
        print("charge avant vente : ",charge)
        ...
        #assert cargo is empty
        updated_ship = self.get(f"/ship/{self.sid}")
        charge_after = updated_ship["cargo"]["resources"]
        for res, amnt in charge_after.items():
            print('res = ',res, 'amount = ', amnt)
            assert amnt == 0.0
        print("charge après vente : ",charge_after)
