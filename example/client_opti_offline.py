...
class Resource:
    def __init__(self, name, price,type,minRank):
        self.name = name
        self.price = price
        self.type = type
...
class Game:
    ...
    def go_mine(self):
        ...
        station = self.get(f"/station/{self.sta}")
        if check_has(station["crew"], "member_type", "Operator"):
            print("[*] OPERATOR RANK:", station["crew"]["Operator"])
            rank = station["crew"]["Operator"]["rank"]
        else:
            rank = 0
            print("[*] No Operator found, rank is set to 0")
        # Scan the most optimized resource to farm for its distance and price
        
        # get the current price of the resource
        baseResources = self.get(f"/resources")
        # Get the list of resources available on the market and its price
        market = self.get(f"/market/prices")
        print("[*] Available resources on the market:")
        mostProfitableResource = Resource('',0,{},0)
        for resource, price in market["prices"].items():
            # Skip the resources we don't want to farm
            if resource == "Fuel" or resource == "HullPlate":
                pass
            # we catch the base price of the resource
            basePrice = baseResources[resource]["base-price"]
            print(f"\t- {resource}: {price} credits (base price: {basePrice} credits)")
            
            # #ensure we can mine the most profitable resource
            if mostProfitableResource.minRank> rank:
                print(f"[*] We can't mine {resource} because our rank is {rank} and the minimum rank is {mostProfitableResource.minRank}")
                continue
            
            if basePrice < price :
                if mostProfitableResource.name == "":
                    mostProfitableResource = Resource(resource, price,baseResources[resource],baseResources[resource]["min-rank"])
                
                elif mostProfitableResource.price < price:
                    mostProfitableResource = Resource(resource, price,baseResources[resource],baseResources[resource]["min-rank"])
        if mostProfitableResource.type == 'solid':
            mostProfitableResource.type = True
        else:
            mostProfitableResource.type = False
        ...
        planetsDistance = sorted(planets,
            key=lambda pla: get_dist(station["position"], pla["position"])
        )
        for planet in planetsDistance :
            if planet["solid"] == mostProfitableResource.type:
                nearest = planet
                break
    ...
    def go_sell(self):
        ...
        print(f"[*] SHOP UPGRADE: {self.get(f'/station/{self.sta}/shop/modules/{self.sid}/upgrade')}")
        upgradePref = []
